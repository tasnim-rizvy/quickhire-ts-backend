...
/**
 * Validates company logo: accepts either base64 data URIs, valid HTTP(S) URLs, or empty string (no logo)
 */
const isValidCompanyLogo = (value: string): boolean => {
  // Empty string is valid (represents no logo - can be used to remove logo)
  if (!value) return true;

  // Check for base64 data URI format
  if (value.startsWith('data:image/')) {
    return /^data:image\/(png|jpeg|jpg|webp|svg\+xml);base64,.+/.test(value);
  }
...
export const validateCreateJob: (
	...
	body('company_logo')
		.optional()
		.trim()
		.custom(isValidCompanyLogo)
		.withMessage('Company logo must be either a valid URL (http/https) or a base64 encoded image'),
	...
	body('category')
		...
		.withMessage(
			`Category must be one of: ${Object.values(JobCategory).join(', ')}`,
		),
